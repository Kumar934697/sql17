...
use zomato;
CREATE TABLE orders (id INT PRIMARY KEY,order_name VARCHAR(25),cost BIGINT(20),created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,created_by VARCHAR(25),modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,modified_by VARCHAR(25)
);
...
CREATE TABLE payment (id INT PRIMARY KEY,price bigint(20),payment_id INT,order_id INT,payment_status VARCHAR(25),created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,created_by VARCHAR(25),modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,modified_by VARCHAR(25),FOREIGN KEY (order_id) REFERENCES orders(id));
...
CREATE TABLE restaurant (id INT PRIMARY KEY,restaurant_name VARCHAR(25),restaurant_id INT,location VARCHAR(25),order_id INT,ratings int(20),payment_id INT,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,created_by VARCHAR(25),modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,modified_by VARCHAR(25),FOREIGN KEY (order_id) REFERENCES orders(id),FOREIGN KEY (payment_id) REFERENCES payment(id)
);
...
CREATE TABLE delivery (id INT PRIMARY KEY,person_name VARCHAR(25),mode_of_payment ENUM('Cash', 'Card', 'Online'),order_id INT,restaurant_id INT,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,created_by VARCHAR(25),modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,modified_by VARCHAR(25),FOREIGN KEY (order_id) REFERENCES orders(id),FOREIGN KEY (restaurant_id) REFERENCES restaurant(id));
